...
export default function TherapistViewPage() {
  ...
  const [therapistId, setTherapistId] = useState(params?.id);
  ...
  const getDateFilterString = () => {
    ...
    if (dateFilter === 'today') {
      ...
      filterStr = `date >= "${formatISO(start)}" && date <= "${formatISO(end)}"`;
  ...
  const fetchBookingRequests = async () => {
    ...
    try {
      ...
      const statusStr = statusArr.map(s => `status="${s}"`).join('||');
      const dateStr = getDateFilterString();

      let filter = `therapist_id="${therapistId}"`;
  ...
  }, [profileExists, therapistId, dateFilter, customDate, statusFilter]);
  ...
  // Remove duplicate fetchData and profileExists logic
  useEffect(() => {
    if (!profileExists) {
      setIsEditing(true);
    } else {
  ...
  console.log("Booking Request ", bookingRequests)
                ...
                {isEditing ? (
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                    {username ? (
                      ...
                      <p className="text-gray-400 italic">No username set</p>
                    )}
                  </div>
                )}
              </div>
